...
datos_xls <- eventReactive(input$analisis, {  
  ...
  percent_mayor1 <- round(nrow(desv_sup1) * 100 / nrow(datos2), digits = 2)
  ...
  datos_csv <- eventReactive(input$analisis, {
    ...
    datos1 <- datos %>%
      ...
      mutate(Fecha = case_when(estufas == "2005" | estufas == "488" ~ mdy_hms(Fecha),
                               .default = dmy_hms(Fecha)) , Temperatura = as.double(Temperatura)) %>% drop_na()
    ...
    inicio <- datos1$Fecha[1]
    
    fin <-  datos1$Fecha[length(datos1$Fecha)]
